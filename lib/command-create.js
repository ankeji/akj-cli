...
const { downloadSuccessfully } = require('./downloadSuccessfully');
...
function downloadTemplate(projectInfo, template) {
    ...
    const downloadObj = templateList.find(e=>e.name == template)

    if (downloadObj) {
        const downloadUrl = templateList.find(e=>e.name == template).link
        if (projectName) {
            download(downloadUrl, projectName, { clone: true }, function (error) {
                if (!error) {
                    editFile({ version, projectName, authorName }).then(() => {
                        ...
                        console.log('✌✌✌', chalk.rgb(0, 0, 255)('成功创建项目：'), chalk.rgb(0, 255, 127)(projectName))
        ...
        console.log(chalk.bgRgb(220, 0, 8)(`  创建项目失败：${projectName} `), '   ')
        console.log('    失败原因：', chalk.bgRgb(220, 0, 8)('模板不存在！'))
    }
}
